getStErrorReals<- function(inData, inb0){
  
  #set up dataset
  catList = levels(as.factor(ENROLL_DATA$cat))
  transData = data.frame(t(inData))
  transData = transData[,1:8]
  colnames(transData)= c(19:26)
  ...
  for (i in 1:length(catList)){
    ...
    if (nrow(curData) > 1){ #if there are at least two people in this category
      avgVect = rep(NA, 8)
      outData = data.frame(matrix(ncol = 8, nrow = nrow(curData)))
      colnames(outData) = colnames(curData)[1:8]
